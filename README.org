The voxin installer installs voxin and its dependencies on various GNU/Linux distributions (e.g. Arch Linux, Debian, Fedora, Slackware, Slint, Ubuntu,...). 

The current goal is to provide:
- distribution-independent tarballs (in /opt directory), 
- a few minimal and stable debian packages, easily converted to other package format (/usr, ...), 
...
The rootfilesystem can be (re)built if needed; it is a relatively intensive process (more than one hour, 3.5 GB of storage space).
