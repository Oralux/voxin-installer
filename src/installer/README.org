...
Add the following function to your .bashrc to make the previous examples work with the local voxin installation.

#+BEGIN_EXPORT ascii
voxin-say() {
  local DIR=$HOME/.local/share/voxin/rfs/opt/oralux/voxin
  LD_LIBRARY_PATH=$DIR/lib $DIR/bin/voxin-say $@
}
#+END_EXPORT
